...
export {
  # constant hypervectors representing which endpoint wrote the record
...
redef record SSL::Info += {
  # ordered roll-filler hvs: bind(endpoint, length, interval)
...
event ssl_encrypted_data(c: connection, is_client: bool, record_version: count, content_type: count, length: count) {
  ...
  c$ssl$roll_filler_hvs[|c$ssl$roll_filler_hvs|] = VSA::bind(vector(
...
function make_ngram_bundle(hvs: vector of hypervector): hypervector {
...
event connection_state_remove(c: connection) {
  if (! c?$ssl) { return; }
  ::conns_as_ngram_bundle[|::conns_as_ngram_bundle|] = make_ngram_bundle(c$ssl$roll_filler_hvs);
