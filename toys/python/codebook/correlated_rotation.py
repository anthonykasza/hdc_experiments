...
def representation_shift(codebook, elements=5):
  '''All symbols in the codebook have the same randomly selected elements shifted'''
  new_codebook = {}

  # The same indices are rotated across hypervectors
  item = list(codebook.values())[0]
  all_indices = list(range(len(item)))
  to_change = [int(x) for x in np.random.choice(all_indices, int(elements), replace=False)]
