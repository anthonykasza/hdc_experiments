...
for idx in range(len(levels)):
  cv2.imwrite(f'./output/levels/shoes_to_pen_{idx:03d}.jpg', levels[idx].reshape((512, 512)))

vec_pen = img_pen.flatten().astype(np.int16)
vec_noise = img_noise.flatten().astype(np.int16)
levels = make_levels_manhattan(10, vec_pen, vec_noise)
for idx in range(len(levels)):
  cv2.imwrite(f'./output/levels/pen_to_noise_{idx:03d}.jpg', levels[idx].reshape((512, 512)))
