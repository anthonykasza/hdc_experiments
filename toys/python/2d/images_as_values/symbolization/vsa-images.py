...
# ------------------------------------------------------------
# Image preprocessor. Make a bw image match a VSA's ensemble
# ------------------------------------------------------------

def transf(img):
  '''
  I tried both histogram specification and spectral whitening
  and the latter appears to work better.

  https://en.wikipedia.org/wiki/Histogram_matching#Example
...
pen = transf(raw_pen)
shoes = transf(raw_shoes)
noise = transf(raw_noise)
...
#  a transform, such as a high-boost sharpening filter
...
for r, (name, raw, stable) in enumerate(pairs):
    ...
    ax[r, 1].set_title(f"Transformed {name}")
