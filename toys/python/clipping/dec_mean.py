...
from clipping_strategies import clip_dec_mean
...
cb = CountingBundle(rate=3, clip_func=clip_dec_mean, n=5)
...
tb = ThresholdBundle(thresh=5, clip_func=clip_dec_mean, n=5)
for idx in range(len(signals)):
  s = signals[idx]
...
bb = BatchBundle(clip_func=clip_dec_mean, n=5)
for idx in range(len(signals)):
  s = signals[idx]
